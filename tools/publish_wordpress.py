...
def make_post_payload(
    data: dict[str, Any], html: str, category_ids: list[int], tag_ids: list[int]
) -> dict[str, Any]:
...
class WordPressClient:
    ...
    def ensure_term(self, taxonomy: str, name: str) -> int:
        ...
        terms = self.request("GET", endpoint, params={"search": name, "per_page": 100})
    ...
    def upsert(self, data: dict[str, Any], html: str) -> dict[str, Any]:
        category_names = list(
            (data.get("wordpress") or {}).get("categories") or data.get("categories") or []
        )
        tag_names = list(
            (data.get("wordpress") or {}).get("tags") or data.get("tags") or []
        )
